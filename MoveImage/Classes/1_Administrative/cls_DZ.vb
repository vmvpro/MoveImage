...
Public Class cls_DZ
    ...
    Public Sub New()
        ...
        type_doc = "Докладная записка"
        'type_docRange = New String() {"Служебная записка"}
        dirName = "1_Административная документация\Докладные записки\"
    ...
    Protected Overrides Function inputFields() As Boolean
        ...
        If (isValidText(class02_num, " ")) Then
            MessageBox.Show("Постарайтесь ввести обозначение Докладной записки без пробелов", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Return False
        End If
    ...
    Protected Overrides Sub defineName()
        ...
        mas_SaveImages = New String() {"ДЗ ", temp02_num, " от ", temp11_dates}
        mas_SaveFolder = New String() {"ДЗ ", temp02_num, " от ", temp11_dates, temp18_txt1}
        mas_FullPathFolder = New String() {field11_dates.Value.Year & "\", "ДЗ ", temp02_num, " от ", temp11_dates, temp18_txt1}
